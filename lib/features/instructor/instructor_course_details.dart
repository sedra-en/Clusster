...
class _InstructorCoursesScreenState extends State<InstructorCoursesScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("موادي الدراسية"), backgroundColor: Colors.transparent, elevation: 0),
      body: AppBackground(
        child: _isLoading 
  ...
  Widget _buildCourseCard(dynamic course) {
    final color = AppColors.primary; // يمكنك استخراج اللون من الحقل cover_color لاحقاً
    return ScaleButton(
      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => UploadLectureScreen(
        courseId: course['id'].toString(), // ✅ تمرير الـ ID المطلوب
        courseTitle: course['title'],
        color: color,
      ))),
      child: Container(
        decoration: BoxDecoration(color: getCardColor(context), borderRadius: BorderRadius.circular(20), border: Border.all(color: color.withOpacity(0.2))),
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.book_rounded, color: color, size: 40),
          const SizedBox(height: 10),
          Text(course['title'], textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold)),
        ]),
      ),
    );
  }
